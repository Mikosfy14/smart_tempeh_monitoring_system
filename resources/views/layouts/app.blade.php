...
<head>
    ...
    <title>@yield('title', 'Dashboard — Rizhomatix')</title>
...
<body class="min-h-screen">
    ...
    <nav class="nav-bar">
        ...
        <div class="nav-logo">
            ...
            <span class="hidden sm:block">Rizhomatix</span>
            <span class="sm:hidden text-sm">Rizhomatix</span>
        </div>

        {{-- Right Group (Add Device, Profile, Logout) --}}
        <div class="flex items-center gap-1 sm:gap-4 ml-auto">
            <button class="btn btn-primary btn-sm px-2 sm:px-4" onclick="openModal('register-modal')" id="btn-add-device" title="Tambah Alat">
            ...
            <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 text-sm font-medium px-2" style="color: var(--color-text-secondary); text-decoration: none;" id="btn-profile-link" title="Profile">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <form method="POST" action="{{ route('logout') }}" class="m-0">
                @csrf
                <button type="submit" class="btn btn-secondary btn-sm px-2 sm:px-4" id="btn-logout" title="Logout">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
