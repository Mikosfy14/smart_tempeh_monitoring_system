        ...
        <table class="data-table" id="whitelist-table">
            ...
            <tbody>
                ...
                <tr id="row-{{ $master->id }}">
                    ...
                    <td><code class="text-sm px-2 py-1 rounded" style="background: var(--color-bg-primary); color: var(--color-accent-teal);">{{ $master->device_id }}</code></td>
                    <td>
                    ...
                    <td style="color: var(--color-text-secondary);">{{ $master->created_at->format('d M Y') }}</td>
                    <td>
                        <div class="flex items-center gap-1">
                            @if(!$master->is_registered)
                                ...
                                <button class="btn-icon" style="color: var(--color-accent-teal);" onclick="openAssignModal({{ $master->id }}, '{{ $master->device_id }}')" title="Assign to User">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" /></svg>
                                </button>
                            ...
                            @else
                                ...
                                <button class="btn-icon" style="color: var(--color-accent-amber);" onclick="unassignDevice({{ $master->id }}, '{{ $master->device_id }}')" title="Unassign from User">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 7a4 4 0 11-8 0 4 4 0 018 0zM9 14a6 6 0 00-6 6v1h12v-1a6 6 0 00-6-6zM21 12h-6" /></svg>
                                </button>
                            @endif
                        </div>
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-8" style="color: var(--color-text-muted);">No hardware IDs in the whitelist.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div class="modal-overlay" id="unassign-modal">
    <div class="modal-content text-center">
        <div class="inline-flex items-center justify-center w-14 h-14 rounded-full mb-4" style="background: rgba(251, 191, 36, 0.1);">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7" style="color: var(--color-accent-amber);" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
        </div>
...
async function addDevice(e) {
    ...
    const body = { device_id: document.getElementById('add-device-id').value.toUpperCase() };
...
async function confirmUnassign() {
    ...
    try {
        ...
        location.reload();
    } catch (err) { alert('Failed to unassign device.'); }
}
</script>
@endpush
